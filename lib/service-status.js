...
// Avisa a los usuarios que preguntaron (/notas o /estado en el bot) mientras
// INTRALU estaba caído — quedaron anotados en service_status_interesados
// (ver telegram-webhook/index.ts, avisoIntraluCaido()). A diferencia del
// aviso al admin, esto es best-effort por usuario: si uno falla (bloqueó al
// bot, chat_id inválido) no debe frenar el resto. Siempre vacía la tabla al
// final, haya o no interesados, para no arrastrar filas viejas a la próxima
// caída.
async function notificarInteresados(supabase, telegramToken, msg) {
  const { data: interesados, error } = await supabase
    .from('service_status_interesados')
    .select('chat_id')
    .eq('service', 'intralu');

  if (error) {
    console.error('notificarInteresados:', error.message);
  } else if (interesados && interesados.length > 0) {
    await Promise.all(interesados.map((row) => sendTelegram(telegramToken, row.chat_id, msg).catch(() => {})));
  }

  await supabase.from('service_status_interesados').delete().eq('service', 'intralu');
}
...
// tiene sentido avisar que se recuperó (tampoco debería haber interesados
// anotados en ese caso, pero se limpia la tabla igual por las dudas).
async function markIntraluUp(supabase, telegramToken, adminChatId) {
  ...
  console.log(msg);
  if (adminChatId) await sendTelegram(telegramToken, adminChatId, msg).catch(() => {});
  await notificarInteresados(supabase, telegramToken, `${msg} Ya puedo revisar tus notas de nuevo.`);
}
